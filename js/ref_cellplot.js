...
CellPlot=function(container){
	...
	if(this.canvas.error){
		...
		htmlerror.innerHTML="Sorry, WebGLRender init fault. Seems your brower do not support webgl. <br> Try to access <a href=\"http://get.webgl.org/\">http://get.webgl.org </a> to have a test of the WebGl's support. <br> Also you may need to use <a href=\"http://windows.microsoft.com/en-us/internet-explorer/download-ie\">IE11</a> or <a href=\"http://www.mozilla.org/en-US/firefox/new/\" >FireFox </a>instead.<br> Here is some useful information: <br><br>When you are using chrome, go to chrome:\/\/gpu to see whether webgl is supported. If so, try to restart chrome or reload the page to see whether the page works. Otherwise, try to check info in chrome:\/\/flags .<br><br> When you are using Chromium under linux and webgl can't work, <a href=\"http://askubuntu.com/questions/299345/how-to-enable-webgl-in-chrome-on-ubuntu\">This page </a> may help you.<br>When you are using safari, maybe you need to open webgl manually.<a href=\"http://onvert.com/guides/enable-webgl-safari/\">Here </a> are some methods to enable it.<br><br> Please email me ( i@catofes.com ) for more infomation.";
...
CellPlot.Canvas.prototype.LoadData=function(para)
{
	...
	if (!current_cellids[current_tloc]){// no data, need to load data
		fpath="./ajaxfolder/refmove_"+refid+"/g0_t"+current_tloc+".txt";
	...
	if(this.showseg){
		if (!segp_locs[current_tloc]){
			var fpath="./ajaxfolder/refmove_"+refid+"/s0_t"+current_tloc+".txt";
...
CellPlot.Canvas.prototype.ByGeneExp=function()
{
	...
	for (var c=0; c<cellnum;c++){
		var tempcolor=[0,0,0];
		if(isNaN(geneexp[geneid][c]))// not a number
		  tempcolor=[100,100,100];
		else{
			for (var k=0; k<3;k++){
				tempcolor[k]=Math.max(1,Math.min(255,Math.floor(300*(colortable[Math.floor(geneexp[geneid][c]*63)][k]))));
			}
...
CellPlot.Tree.prototype.LoadData=function(name)
{
	...
	for(i=0;i<dlist.length;i++){
		...
		color=[];	
		for ( k=0; k<3;k++)
		  if (isNaN(geneexp[geneid][cellid])===false)
			color[k]=Math.min(255,Math.floor(300*(colortable[Math.floor(geneexp[geneid][cellid]*63)][k])));
		  else
			color[k]=200;
...
CellPlot.Control.prototype.SelectbyName=function(name)
{
	...
	for(i=0;i<splitname.length;i++){
		loc=cellnames.indexOf(splitname[i]);
